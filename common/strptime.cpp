...
{
	const char *p_start=p_file_name, *p_temp;
	const char *p_end=p_start+strlen(p_start);
	int iloop=128;
	while(iloop--)
	{
		p_temp=strstr(p_start, "20");
		if(p_temp==NULL)
		{
			return false;
		}
		...
		{
			if(strptime(p_temp, "%Y%m%d%H%M%S", p_tm))
			{
				return true;
			}
		}
...
{
	if(!GetTMFromFileName(p_file_name, p_tm))
	{
		return false;
	}
	p_tm->tm_year+=TM_YEAR_BASE;
	p_tm->tm_mon+=1;
	return true;
}
...
bool GetTimeFromFileName(const char *p_file_name, unsigned long *p_time)
{
	...
	if(!GetTMFromFileName(p_file_name, &temp_tm))
	{
		return false;
	}
	t_of_day=mktime(&temp_tm);
	if(t_of_day<0)
	{
		return false;
	}
	*p_time=(unsigned long)t_of_day;
	return true;
}
...
bool GetTimeFromFileName(const char *p_file_name, unsigned long long *p_time)
{
	...
	*p_time=(unsigned long long)t_of_day;
	return true;
}
