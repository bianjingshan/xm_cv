...
public:
   ...
   XMText(void);
   XMText(const char *freeType);
   void init(const char *freeType);
   virtual ~XMText();

   void setFont(int font_size=20, bool underline=0, float diaphaneity=1.0);
   void setColor(const Scalar color);
   void putTextMat(Mat &input_image, Point o_pt, const char *p_msg_buffer);
   void putTextMat(Mat &input_image, Point o_pt, const wchar_t *p_msg_buffer);
   int putTextMat(Mat &input_image, Rect o_postion, const char *p_msg_buffer);
   void getTextSize(char *p_msg_buffer, Size &text_size);
...
private:
   ...
   int m_font_size;
   wchar_t m_caption[10240];
   wchar_t temp_buffer[1024];
};
